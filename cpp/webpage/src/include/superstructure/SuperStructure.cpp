...
void SuperStructure::loop(){
    ...
    while(true){
        ...
        cv::putText(latestFrame, std::to_string(camera.getFramerate()), cv::Point(0, 40), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 0), 2, cv::LINE_AA);

        //code goes here
        detector.process(&latestFrame);

        if(!camera.show(latestFrame))
            break;
